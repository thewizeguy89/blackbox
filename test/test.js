...
contract('Blackbox', ([deployer, uploader]) => {
  let blackbox

  before(async () => {
    blackbox = await Blackbox.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await blackbox.address
    ...
    it('has a name', async () => {
      const name = await blackbox.name()
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await blackbox.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await blackbox.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await blackbox.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await blackbox.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await blackbox.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await blackbox.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await blackbox.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await blackbox.files(fileCount)
